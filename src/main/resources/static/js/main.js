...
function login(e) {
  ...
  else { Toast.show("Email atau password salah.", 'error'); }
  return false;
}
...
function register(e) {
  ...
  const users = load('users', []);
  if (users.some(u => u.email === email)) { Toast.show('Email sudah terdaftar.', 'error'); return false; }
  users.push({ name, email, password }); save('users', users);
  Toast.show('Registrasi berhasil. Silakan login.', 'success'); window.location.href = "login.html"; return false;
}
...
function initSongDetail() {
  ...
  document.getElementById('btnLike').onclick = () => {
    const u = currentUser(); if (!u) { Toast.show('Silakan login terlebih dahulu.', 'warning'); return; }
  ...
  document.getElementById('btnRepeat').onclick = () => { const p = document.getElementById('player'); p.loop = !p.loop; Toast.show(p.loop ? 'Repeat aktif' : 'Repeat nonaktif', 'info'); };
  ...
  send.onclick = () => { const u = currentUser(); if (!u) { Toast.show('Silakan login.', 'warning'); return; } const v = input.value.trim(); if (!v) return; song.comments.push({ user: u.name, text: v }); save('songs', songs); input.value = ''; renderComments(song.id); };
}
...
function addToPlaylistPrompt(songId) {
  const pls = userPlaylists(); if (pls.length === 0) { Toast.show('Buat playlist dulu ya.', 'info'); return; }
  ...
  if (!pls[idx].ids.includes(songId)) pls[idx].ids.push(songId); saveUserPlaylists(pls); Toast.show('Ditambahkan ke playlist.', 'success');
}
...
function deletePlaylist(i) {
  ...
  Dialog.confirm('Hapus Playlist', 'Apakah Anda yakin ingin menghapus playlist ini?', 'Hapus', 'Batal', () => {
    pls.splice(i, 1);
...
function submitFeedback(e) {
  ...
  renderFeedbackList();
  Toast.show('Feedback terkirim (mock).', 'success');
  return false;
}
...
function initAdmin() {
  ...
  if (!isAdmin()) { Toast.show('Khusus admin.', 'error'); window.location.href = 'index.html'; return; }
...
function adminDelete(id) {
  Dialog.confirm('Hapus Lagu', 'Apakah Anda yakin ingin menghapus lagu ini?', 'Hapus', 'Batal', () => {
